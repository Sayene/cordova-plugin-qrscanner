...
class QRScanner : CDVPlugin, AVCaptureMetadataOutputObjectsDelegate {
    ...
    override func pluginInitialize() {
        ...
        self.cameraView = UIView(frame: CGRect(x: 0, y: 0, width: UIScreen.main.bounds.width, height: (UIScreen.main.bounds.height * 0.66)))
    }
    ...
    func configureLight(command: CDVInvokedUrlCommand, state: Bool){
        ...
        do {
            if(backCamera!.hasTorch == false || backCamera!.isTorchAvailable == false || backCamera!.isTorchModeSupported(useMode) == false){
    ...
    func captureOutput(_ captureOutput: AVCaptureOutput!, didOutputMetadataObjects metadataObjects: [Any]!, from connection: AVCaptureConnection!) {
        ...
        if found.stringValue != nil {
            scanning = false
            let scanResult: [String : String] = ["metatype": found.type, "scanString": found.stringValue]
            AudioServicesPlaySystemSound(SystemSoundID(kSystemSoundID_Vibrate))
            let pluginResult = CDVPluginResult(status: CDVCommandStatus_OK, messageAs: scanResult)
    ...
    func getStatus(_ command: CDVInvokedUrlCommand){
        ...
        let status = [
            ...
            "webviewBackgroundIsTransparent": boolToNumberString(bool: webviewBackgroundIsTransparent),
